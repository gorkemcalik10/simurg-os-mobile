(function(root,factory){
  ...
  var runtime={initialized:false,initializing:null,backend:'legacy_localstorage_fallback',migrationStatus:'not_started',db:null,indexedDB:null,storage:null,prepare:null,indexedDbBytes:0,lastError:null};
  ...
  function stateSnapshot(){return {backend:runtime.backend==='indexeddb'?'IndexedDB':'Legacy localStorage fallback',backendCode:runtime.backend,migrationStatus:runtime.migrationStatus,indexedDbBytes:runtime.indexedDbBytes,lastError:runtime.lastError}}
  ...
  async function initialize(options){
    ...
    runtime.initializing=(async function(){
      var legacy=readLegacy(runtime.storage),dbError=null,invalidRecord=null;
      try{
      ...
      runtime.db=null;runtime.backend='legacy_localstorage_fallback';runtime.migrationStatus='fallback_due_to_error';runtime.initialized=true;rememberError(dbError);
      return {ok:true,data:legacy.ok?legacy.data:null,source:legacy.ok?'legacy_localstorage':'default',legacyError:legacy.error||null,error:dbError,state:stateSnapshot()};
    })();
    return runtime.initializing;
  }
  async function persistData(storage,value,options){if(typeof globalThis!=='undefined'&&globalThis.__simurgStartupDataRecoveryActive)return recoveryFailure();options=options||{};if(runtime.initializing)await runtime.initializing;if(runtime.backend==='indexeddb'&&runtime.db){try{return await writeCanonical(value,options.source||'save')}catch(error){return failure(MAIN_KEY,error,'indexeddb_write_failed')}}var raw;try{raw=JSON.stringify(value)}catch(error){return failure(DATA_KEY,error)}var preflight=inspectStorage(storage||runtime.storage,raw),result=writeRaw(storage||runtime.storage,DATA_KEY,raw);result.diagnostics=preflight;result.backend='legacy_localstorage_fallback';return result}
  function resetForTests(){if(runtime.db&&typeof runtime.db.close==='function')runtime.db.close();runtime={initialized:false,initializing:null,backend:'legacy_localstorage_fallback',migrationStatus:'not_started',db:null,indexedDB:null,storage:null,prepare:null,indexedDbBytes:0,lastError:null}}
  return {DATA_KEY:DATA_KEY,DB_NAME:DB_NAME,DB_VERSION:DB_VERSION,STORE_NAME:STORE_NAME,MAIN_KEY:MAIN_KEY,PENDING_KEY:PENDING_KEY,STORAGE_VERSION:STORAGE_VERSION,BACKUP_KEYS:BACKUP_KEYS.slice(),utf8Bytes:utf8Bytes,stableSerialize:stableSerialize,checksum:checksum,inspectStorage:inspectStorage,diagnostics:diagnostics,state:stateSnapshot,initialize:initialize,persistData:persistData,writeRaw:writeRaw,writeJson:writeJson,remove:remove,requireSuccess:requireSuccess,notifyFailure:notifyFailure,_resetForTests:resetForTests,_verifyRecord:verifyRecord};
});
