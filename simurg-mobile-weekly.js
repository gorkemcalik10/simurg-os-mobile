(function(root,factory){
  ...
  function firstNumber(){for(var index=0;index<arguments.length;index+=1){var value=number(arguments[index]);if(value!=null)return value;}return null;}
  ...
  function dailyRecord(data,key,date){var value=data&&data[key]&&data[key].daily&&data[key].daily[date];return value&&typeof value==='object'&&!Array.isArray(value)?value:null;}
  ...
  function buildWeek(start,options){
    ...
    var polarDays=days.map(function(day){return {date:day.date,result:resolvedPolar(deps.polar,day.date,deps.data),nightly:dailyRecord(deps.data,'polarNightlyRecharge',day.date),activity:dailyRecord(deps.data,'polarActivity',day.date)};}),sleepDays=days.map(function(day){return {date:day.date,result:resolvedSleep(deps.sleep,day.date,deps.data)};});
    function polarMetric(reader){var values=polarDays.map(function(item){try{return number(reader(item.result,item));}catch(error){return null;}}).filter(function(value){return value!=null;});return {value:round(average(values),1),sampleSize:values.length};}
    function polarTotal(reader){var values=polarDays.map(function(item){try{return number(reader(item.result,item));}catch(error){return null;}}).filter(function(value){return value!=null;});return {value:values.length?round(values.reduce(function(sum,value){return sum+value;},0),1):null,sampleSize:values.length};}
    var hrvMetric=polarMetric(function(result,item){return firstNumber(result&&result.nightly&&result.nightly.metrics&&result.nightly.metrics.hrv,item.nightly&&item.nightly.heartRateVariabilityAvg);}),nightHrMetric=polarMetric(function(result,item){return firstNumber(result&&result.nightly&&result.nightly.metrics&&result.nightly.metrics.nightHr,item.nightly&&item.nightly.heartRateAvg);}),stepsMetric=polarTotal(function(result,item){return firstNumber(result&&result.activity&&result.activity.metrics&&result.activity.metrics.steps,item.activity&&item.activity.steps);}),activeMetric=polarTotal(function(result,item){return firstNumber(result&&result.activity&&result.activity.metrics&&result.activity.metrics.activeMinutes,item.activity&&item.activity.activeMinutes);});
    ...
    var isActive=start===currentMonday,officialLoadDays=days.filter(function(day){return day.load&&day.load.available&&day.load.sourceType==='official'&&number(day.load.value)!=null;}),loadValues=officialLoadDays.map(function(day){return number(day.load.value);}),latestLoadDay=officialLoadDays.slice(-1)[0]||null,latestLoad=latestLoadDay&&latestLoadDay.load||null,cardioLoadValue=loadValues.length&&(isActive||loadValues.length>=3)?round(average(loadValues),1):null;
    ...
    return {
      status:'available',startDate:start,endDate:bounds.end,safeEndDate:bounds.safeEnd,dates:bounds.dates,coverageDays:bounds.dates.length,isActive:isActive,isPartial:bounds.safeEnd<bounds.end,canonical:canonical,hasEarlierData:hasEarlierTrustedData(deps.data,start),
  ...
  function compare(current,previous){
    ...
    return {
      ...
      trainingDays:period('trainingDays',current&&current.training.days,previous&&previous.training.days,'integer'),
  ...
  function tonLabel(value){return round(value,1).toLocaleString('tr-TR',{minimumFractionDigits:1,maximumFractionDigits:1});}
  function volumeDisplay(value){value=number(value);return value==null?{value:'—',unit:''}:value>=1000?{value:tonLabel(value/1000),unit:'ton'}:{value:numberLabel(value),unit:'kg'};}
  ...
  function primaryCard(label,value,delta,support,className){return '<article class="mwPrimary '+className+(delta?'':' current')+'"><small>'+escapeHtml(label)+'</small><strong>'+escapeHtml(value)+'</strong><span>'+escapeHtml(support||'')+'</span>'+(delta?'<em>'+escapeHtml(delta)+'</em>':'')+'</article>';}
  function metricCard(label,value,unit,delta,sample,context,className){if(value==null)return '';return '<article class="mwMetric'+(className?' '+escapeHtml(className):'')+'"><small>'+escapeHtml(label)+'</small><b>'+escapeHtml(value)+(unit?' <i>'+escapeHtml(unit)+'</i>':'')+'</b>'+(delta?'<span>'+escapeHtml(delta)+'</span>':'')+(sample?'<em>'+escapeHtml(sample)+'</em>':'')+(context?'<em class="mwMetricContext">'+escapeHtml(context)+'</em>':'')+'</article>';}
  ...
  function rhythm(current){
    var source=current&&current.canonical&&list(current.canonical.days)||[],byDate={};source.forEach(function(day){if(day&&day.date)byDate[day.date]=day;});
    var values=source.map(function(day){return day&&day.load&&day.load.available&&day.load.sourceType==='official'?number(day.load.value):null;}).filter(function(value){return value!=null;}),maximum=Math.max.apply(null,values.concat([1]));
    var labels=['Pzt','Sal','Çar','Per','Cum','Cmt','Paz'];
    return '<section class="mwRhythm" aria-label="Yedi günlük antrenman ve yük ritmi"><header><div><small>7 GÜNLÜK RİTİM</small><h2>Antrenman ve yük</h2></div><span>Mevcut günlük kayıtlar</span></header><div class="mwRhythmPlot">'+[0,1,2,3,4,5,6].map(function(offset){
      var date=addDays(current.startDate,offset),day=byDate[date],covered=current.dates.indexOf(date)>=0,training=!!(day&&(list(day.sessions).length||list(day.gym&&day.gym.rows).length)),load=day&&day.load&&day.load.available&&day.load.sourceType==='official'?number(day.load.value):null;
      var state=!covered?'missing':training?'training':load!=null?'load':'neutral',height=load!=null?Math.max(18,Math.round(load/maximum*100)):training?32:8;
      var detail=load!=null?'Yük '+decimalLabel(load):training?'Antrenman kaydı':'Kayıt yok';
      return '<div class="mwRhythmDay '+state+'" aria-label="'+escapeHtml(labels[offset]+' · '+detail)+'"><div><i style="height:'+height+'%"></i></div><b>'+labels[offset]+'</b><span>'+date.slice(8,10)+'</span></div>';
    }).join('')+'</div></section>';
  }
  ...
  function render(current,previous,today){
    ...
    var volume=volumeDisplay(current.strength.volume),primary=[
      primaryCard('Antrenman günü',numberLabel(current.training.days),deltaLabel(comparison,'trainingDays',' gün'),current.training.sessions+' seans','days'),
      primaryCard('Toplam süre',durationLabel(current.training.durationMinutes),deltaLabel(comparison,'durationMinutes',' dk'),current.training.durationSampleSize+'/'+current.training.sessions+' seans süreli','duration'),
      primaryCard('Gym hacmi',volume.value,deltaLabel(comparison,'volumePercent','%'),(volume.unit?volume.unit+' · ':'')+current.strength.sessions+' Gym seansı','volume')
    ];
    var strength=metricCard('Gym seansı',current.strength.sessions,null,deltaLabel(comparison,'gymSessions',' seans'))+metricCard('Toplam set',current.strength.sets,null,deltaLabel(comparison,'sets',' set'))+metricCard('Toplam tekrar',current.strength.reps,null,deltaLabel(comparison,'reps',' tekrar'))+metricCard('Ort. seans RPE',current.strength.avgRpe==null?null:decimalLabel(current.strength.avgRpe),null,deltaLabel(comparison,'avgRpe',''),current.strength.rpeSampleSize+' seans')+metricCard('PR',current.strength.prCount,null,null,null,null,'mwMetric--inline');
