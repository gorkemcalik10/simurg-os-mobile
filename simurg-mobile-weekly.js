(function(root,factory){
  ...
  function sessionSummary(days){var total=0,gymTotal=0,physical=[];days.forEach(function(day){var gymKeys={},rows=list(day.gym&&day.gym.rows),sessions=uniquePhysicalSessions(day.sessions,day.date);rows.forEach(function(row){gymKeys[gymSessionKey(row,day.date)]=true;});var gymCount=Object.keys(gymKeys).length,strengthPhysical=sessions.filter(isStrengthSession).length,otherPhysical=sessions.length-strengthPhysical;gymTotal+=gymCount;total+=Math.max(gymCount,strengthPhysical)+otherPhysical;physical=physical.concat(sessions);});return {total:total,gym:gymTotal,physical:physical};}
  function rpeAverage(rows){var values=rows.map(function(row){return number(row&&row.rpe);}).filter(function(value){return value!=null;});return {value:round(average(values),1),sampleSize:values.length};}
  ...
  function buildWeek(start,options){
    ...
    var rows=days.reduce(function(output,day){return output.concat(list(day.gym&&day.gym.rows));},[]),rpe=rpeAverage(rows);
    ...
    var durationValues=physicalSessions.map(function(session){return number(session&&session.durationMinutes);}).filter(function(value){return value!=null&&value>0;});
    ...
    var loadValues=days.map(function(day){return day.load&&day.load.available?number(day.load.value):null;}).filter(function(value){return value!=null;}),latestLoadDay=days.filter(function(day){return day.load&&day.load.available;}).slice(-1)[0]||null,latestLoad=latestLoadDay&&latestLoadDay.load||null;
    ...
    return {
      ...
      training:{sessions:sessions.total,durationMinutes:durationValues.length?round(durationValues.reduce(function(sum,value){return sum+value;},0),1):null},
      ...
      polar:{
        ...
        cardioLoad:{value:round(average(loadValues),1),sampleSize:loadValues.length,latestStatusLabel:latestLoad&&latestLoad.statusLabel||null,latestStatusDate:latestLoadDay&&latestLoadDay.date||null}
      },
  ...
  function compare(current,previous){
    ...
    function daily(key,currentMetric,previousMetric,kind){var reason=periodReason;if(!reason&&(!currentMetric||!previousMetric||currentMetric.value==null||previousMetric.value==null))reason='unavailable';if(!reason&&(currentMetric.sampleSize<3||previousMetric.sampleSize<3))reason='insufficient_samples';reasons[key]=reason;return reason?null:difference(currentMetric.value,previousMetric.value,kind);}
    ...
    return {
      ...
      durationMinutes:period('durationMinutes',current&&current.training.durationMinutes,previous&&previous.training.durationMinutes,'integer'),
      ...
      stepsPercent:daily('stepsPercent',current&&current.polar.steps,previous&&previous.polar.steps,'percent'),
      activeMinutes:daily('activeMinutes',current&&current.polar.activeMinutes,previous&&previous.polar.activeMinutes,'integer'),
  ...
  function signed(value,suffix){if(value==null)return 'Karşılaştırma yok';return (value>0?'+':'')+value+(suffix||'');}
  function deltaLabel(comparison,key,suffix){var reason=comparison&&comparison.reasons&&comparison.reasons[key];if(reason==='active_week')return null;if(reason==='insufficient_samples'||reason==='incomplete_week')return 'Yetersiz veri';return signed(comparison&&comparison[key],suffix);}
  ...
  function primaryCard(label,value,delta,unit,className){return '<article class="mwPrimary '+className+(delta?'':' current')+'"><div class="mwPrimaryArc" aria-hidden="true"></div><small>'+escapeHtml(label)+'</small><strong>'+escapeHtml(value)+'</strong><span>'+escapeHtml(unit||'')+'</span>'+(delta?'<em>'+escapeHtml(delta)+'</em>':'')+'</article>';}
  ...
  function summaryText(current,comparison){
    ...
    if(current&&current.isActive){
      sentences.push('Hafta devam ediyor.');
      if(current.strength.sessions)sentences.push('Şu ana kadar '+current.strength.sessions+' Gym seansı ve '+numberLabel(current.strength.volume)+' kg kayıtlı hacim var.');
      else if(current.training.sessions)sentences.push('Şu ana kadar '+current.training.sessions+' antrenman seansı kaydedildi.');
      else sentences.push('Bu haftanın mevcut değerleri gösteriliyor.');
      return sentences.join(' ');
    }
    if(comparison.volumePercent!=null)sentences.push('Gym hacmin geçen haftaya göre %'+Math.abs(comparison.volumePercent)+' '+(comparison.volumePercent>0?'arttı':comparison.volumePercent<0?'azaldı':'aynı kaldı')+'.');
    else if(current.strength.sessions)sentences.push('Bu hafta '+current.strength.sessions+' Gym seansı ve '+numberLabel(current.strength.volume)+' kg kayıtlı hacim var.');
    else if(current.training.sessions)sentences.push('Bu hafta '+current.training.sessions+' antrenman seansı kaydedildi.');
    if(comparison.actualSleepMinutes!=null)sentences.push('Gerçek uyku ortalaman '+Math.abs(comparison.actualSleepMinutes)+' dk '+(comparison.actualSleepMinutes>0?'yükseldi':comparison.actualSleepMinutes<0?'geriledi':'değişmedi')+'.');
  ...
  function render(current,previous,today){
    ...
    var comparison=compare(current,previous),isCurrent=current.startDate===mondayOf(today),nextDisabled=current.startDate>=mondayOf(today),scopeLabel=isCurrent?'Hafta devam ediyor · Haftalık karşılaştırma hafta tamamlandığında oluşacak.':'Tamamlanmış hafta · önceki tamamlanmış hafta';
    ...
    if(current.training.durationMinutes!=null)primary.push(primaryCard('Toplam süre',durationLabel(current.training.durationMinutes),deltaLabel(comparison,'durationMinutes',' dk'),'','duration'));
    if(current.strength.volume!=null)primary.push(primaryCard('Gym hacmi',numberLabel(current.strength.volume),deltaLabel(comparison,'volumePercent','%'),'kg','volume'));
    var strength=metricCard('Gym seansı',current.strength.sessions,null,deltaLabel(comparison,'gymSessions',' seans'))+metricCard('Toplam set',current.strength.sets,null,deltaLabel(comparison,'sets',' set'))+metricCard('Toplam tekrar',current.strength.reps,null,deltaLabel(comparison,'reps',' tekrar'))+metricCard('Toplam hacim',current.strength.volume==null?null:numberLabel(current.strength.volume),'kg',deltaLabel(comparison,'volumePercent','%'))+metricCard('Ortalama RPE',current.strength.avgRpe==null?null:decimalLabel(current.strength.avgRpe),null,deltaLabel(comparison,'avgRpe',''),current.strength.rpeSampleSize+' kayıt')+metricCard('PR',current.strength.prCount||null,null,null);
    var polar=metricCard('Gerçek uyku',current.polar.actualSleepMinutes==null?null:durationLabel(current.polar.actualSleepMinutes),null,deltaLabel(comparison,'actualSleepMinutes',' dk'),current.polar.actualSleepSampleSize+' gece')+metricCard('HRV',current.polar.hrv.value==null?null:decimalLabel(current.polar.hrv.value),'ms',deltaLabel(comparison,'hrv',' ms'),current.polar.hrv.sampleSize+' gece')+metricCard('Night HR',current.polar.nightHr.value==null?null:decimalLabel(current.polar.nightHr.value),'bpm',deltaLabel(comparison,'nightHr',' bpm'),current.polar.nightHr.sampleSize+' gece')+metricCard('Adım',current.polar.steps.value==null?null:numberLabel(current.polar.steps.value),null,deltaLabel(comparison,'stepsPercent','%'),current.polar.steps.sampleSize+' gün')+metricCard('Aktif süre',current.polar.activeMinutes.value==null?null:durationLabel(current.polar.activeMinutes.value),null,deltaLabel(comparison,'activeMinutes',' dk'),current.polar.activeMinutes.sampleSize+' gün')+metricCard('Cardio Load · haftalık ort.',current.polar.cardioLoad.value==null?null:decimalLabel(current.polar.cardioLoad.value),null,deltaLabel(comparison,'cardioLoadPercent','%'),current.polar.cardioLoad.sampleSize+' gün',current.polar.cardioLoad.latestStatusLabel?'Son gün durumu: '+current.polar.cardioLoad.latestStatusLabel:null);
    var visuals=(comparison.volumePercent!=null?visual('Gym hacmi',current.strength.volume,previous&&previous.strength.volume,function(value){return numberLabel(value)+' kg';}):'')+(comparison.durationMinutes!=null?visual('Antrenman süresi',current.training.durationMinutes,previous&&previous.training.durationMinutes,durationLabel):'')+(comparison.actualSleepMinutes!=null?visual('Gerçek uyku',current.polar.actualSleepMinutes,previous&&previous.polar.actualSleepMinutes,durationLabel):'')+(comparison.cardioLoadPercent!=null?visual('Cardio Load',current.polar.cardioLoad.value,previous&&previous.polar.cardioLoad.value,decimalLabel):'');
