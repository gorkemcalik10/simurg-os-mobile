(function(root,factory){
  ...
  function render(current,previous,today){
    ...
    var summary=isCurrent?'':'<section class="mwSummary"><small>SIMURG WEEKLY</small><p>'+escapeHtml(summaryText(current,comparison))+'</p></section>';
    return '<div class="mwShell"><header class="mwHeader"><div><small>'+escapeHtml(isCurrent?'BU HAFTA':'GEÇMİŞ HAFTA')+'</small><h1>Haftalık Özet</h1><p>'+escapeHtml(isCurrent?'Bu haftanın şu ana kadarki değerleri.':'Tamamlanan haftanın önceki tam haftaya göre özeti.')+'</p></div><div class="mwWeekNav"><button type="button" onclick="SimurgMobileWeekly.shift(-1)" aria-label="Önceki hafta" '+(current.hasEarlierData?'':'disabled')+'>‹</button><strong>'+escapeHtml(rangeLabel(current.startDate,current.endDate))+'</strong><button type="button" onclick="SimurgMobileWeekly.shift(1)" aria-label="Sonraki hafta" '+(nextDisabled?'disabled':'')+'>›</button></div><p class="mwComparisonScope '+(isCurrent?'active':'')+'">'+escapeHtml(scopeLabel)+'</p></header><section class="mwPrimaryGrid" aria-label="Haftalık temel metrikler">'+primary.join('')+'</section><section class="mwSection"><header><small>KUVVET</small><h2>Strength özeti</h2></header><div class="mwMetricGrid mwMetricGrid--strength">'+(strength||'<p class="mwUnavailable">Bu hafta Gym kaydı yok.</p>')+'</div></section><section class="mwSection"><header><small>POLAR</small><h2>Haftalık vücut sinyalleri</h2></header><div class="mwMetricGrid mwMetricGrid--polar">'+(polar||'<p class="mwUnavailable">Bu hafta güvenilir Polar özeti için yeterli veri yok.</p>')+'</div></section>'+(visuals?'<section class="mwSection"><header><small>KARŞILAŞTIRMA</small><h2>Bu hafta / Geçen hafta</h2><p>'+escapeHtml(scopeLabel)+'</p></header><div class="mwVisualGrid">'+visuals+'</div></section>':'')+summary+'</div>';
  }
