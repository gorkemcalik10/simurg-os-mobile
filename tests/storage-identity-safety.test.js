...
run('local persistence success returns an explicit success result', () => {
  const storage=memoryStorage(),data={workouts:[{date:'2026-08-14',exercise:'Row'}]};
  const result=persistence.persistData(storage,data);
...
run('failed Gym persistence keeps the draft and never advances the card', () => {
  const body=index.match(/function saveGymExercise\(key\)\{([\s\S]*?)\n\}/)[1];
  assert.match(body,/const result=save\(\)/);assert.match(body,/if\(result\.ok\)\{/);
...
run('UI save returns failure before render or saved timestamp survives', () => {
  const baseSave=index.match(/function save\(\)\{([^\n]+)\}/)[1];
  assert.match(baseSave,/if\(!result\.ok\)/);assert.ok(baseSave.indexOf('if(!result.ok)')<baseSave.indexOf('render()'));
  const wrapped=index.match(/window\.save=function\(\)\{([\s\S]*?)\n  \};/)[1];
  assert.match(wrapped,/hadMeta/);assert.match(wrapped,/previousMeta/);assert.match(wrapped,/if\(!result\.ok\)/);assert.match(wrapped,/if\(hadMeta\)DATA\._meta=previousMeta;else delete DATA\._meta/);
});

run('failed Gym save does not run target persistence or badge success UI', () => {
  const wrapper=index.match(/if\(typeof oldSaveGym==='function'\) window\.saveGymExercise=function\(key\)\{([\s\S]*?)\n  \};/)[1];
  assert.match(wrapper,/if\(ret&&ret\.ok\)/);
  assert.ok(wrapper.indexOf('if(ret&&ret.ok)')<wrapper.indexOf('updateStoredTarget'));
});

run('failed target persistence restores the previous in-memory target', () => {
  const body=index.match(/function updateStoredTarget\(ex,date\)\{([\s\S]*?)\n  \}/)[1];
...
run('custom Gym deletion resolves identity before removing the card metadata', () => {
  const body=index.match(/function deleteGymExercise\(key\)\{([\s\S]*?)\n\}/)[1];
