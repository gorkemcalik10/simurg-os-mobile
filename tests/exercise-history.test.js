...
run('library identity retrieves id records plus compatible legacy names', () => {
  ...
  assert.deepEqual(sessions.map(item => item.date), ['2026-08-15', '2026-08-08', '2026-08-01']);
...
run('history can return only sessions before the selected workout date', () => {
  const sessions = history.sessions(workouts, {
    exerciseId: 'incline_dumbbell_press', name: 'Incline Dumbbell Press', aliases: ['Incline DB Press']
  }, { beforeDate: '2026-08-15', limit: 1 });
  assert.equal(sessions.length, 1);
  assert.equal(sessions[0].date, '2026-08-08');
  assert.deepEqual(sessions[0].summary, { sets: 2, reps: 15, volume: 262.5, best: { weight: 17.5, reps: 8 } });
});
...
run('Gym History loads before the app and uses exercise identity without a schema migration', () => {
  assert.ok(index.indexOf('simurg-exercise-history.js?v=1') < index.indexOf('<script src="./simurg-signal-model.js'));
  assert.match(index, /renderHistoryHtml\(meta\)/);
  assert.match(index, /beforeDate:selectedDate,limit:5/);
  assert.match(sw, /simurg-exercise-history\.js\?v=1/);
