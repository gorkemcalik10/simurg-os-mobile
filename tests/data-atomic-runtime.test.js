...
(async () => {
await run('failed append leaves DATA and localStorage byte-for-byte unchanged', async () => {
  ...
  assert.equal(await window.universalImport(), null);
  assert.equal(JSON.stringify(data), beforeData);
  assert.equal(storage.get('atlas_summary_reports'), beforeRaw);
});

await run('invalid active Apple Watch RPE leaves DATA and localStorage byte-for-byte unchanged', async () => {
  const beforeData = JSON.stringify(data);
  const beforeRaw = storage.get('atlas_summary_reports');
  boxes.universalJsonBox.value = JSON.stringify({
    type: 'activity',
...
await run('quota failure rolls back import and never reports completion', async () => {
  ...
  assert.equal(await window.universalImport(),null);storageFailure=null;
...
await run('render failure after verified persistence does not report a false storage failure', async () => {
  ...
  renderFailures = 1;
  const committed=await window.SimurgDataAtomic.commit(candidate, {
  ...
  assert.notEqual(storage.get('simurg_last_import_snapshot_v1'),'existing-snapshot');
});

await run('successful append validates, snapshots, persists and renders once', async () => {
  data=base();selectedDate='2026-07-23';storage.set('atlas_summary_reports',JSON.stringify(data));
  const beforeRender = renderCount;
  boxes.universalJsonBox.value = JSON.stringify({
    type: 'workout',
    workouts: [{ date: '2026-07-24', exercise: 'Row', sets: 3, reps: 8, weight: 20 }]
  });
  const result = await window.universalImport();
