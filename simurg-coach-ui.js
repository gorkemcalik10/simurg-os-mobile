(function(root){
  ...
  function guidance(result){
    ...
    return '<div class="sci-guidance">'
      +'<article><small>MAIN LIFT</small><p>'+esc(value.mainLifts||'—')+'</p></article>'
      +'<article><small>ACCESSORY</small><p>'+esc(value.accessories||'—')+'</p></article>'
      +'<article><small>STABILITY / POSTURE</small><p>'+esc(value.stabilityPosture||'—')+'</p></article>'
      +'<article><small>CONDITIONING</small><p>'+esc(value.conditioning||'—')+'</p></article>'
      +'</div>';
  }
  function dailyView(date){
    ...
    return '<section class="sci-hero '+statusTone(daily)+'"><div><small>BUGÜNKÜ DURUM</small><h2>'+esc(daily.headline)+'</h2><p>'+esc(daily.summary)+'</p><div class="sci-pills"><span>'+esc(decision(daily))+'</span><span>'+esc(adjustment(daily))+'</span></div></div><div class="sci-score"><b>'+esc(score(daily))+'</b><span>Readiness</span><small>Veri güveni '+esc(confidence(daily))+'</small></div></section>'
      +'<div class="sci-mobile-grid"><section class="sci-card"><header><small>NEDEN?</small><h3>En güçlü sinyaller</h3></header>'+list(daily.keyDrivers,'Henüz güçlü bir driver oluşmadı.',4)+'</section>'
      +'<section class="sci-card sci-action"><header><small>BUGÜN NE YAPMALIYIM?</small><h3>'+esc(decision(pre))+'</h3></header><p>'+esc(pre.workoutGuidance&&pre.workoutGuidance.mainLifts||pre.summary)+'</p><strong>'+esc(adjustment(pre))+'</strong></section></div>'
      +'<section class="sci-card"><header><small>HAREKET REHBERİ</small><h3>Hedef agresifliği</h3></header>'+guidance(pre)+'</section>'
      +(daily.warnings.length?'<section class="sci-card sci-warning"><header><small>RİSK & UYARI</small><h3>Güvenlik katmanı</h3></header>'+list(daily.warnings,'Belirgin uyarı yok.',5)+'</section>':'')
      ...
      +'<details class="sci-details"><summary>Detaylı Analiz</summary><div><h3>Benzer günler</h3>'+list(daily.comparisonNotes,'Yeterince benzer geçmiş gün bulunamadı.',4)+'<h3>Antrenman sonrası</h3><p>'+esc(post.summary)+'</p><h3>Güvenlik notu</h3><p>'+esc(daily.medicalDisclaimer)+'</p></div></details>';
  }
  function weeklyView(date){
    var weekly=resolve('weekly',date);
    return '<section class="sci-hero '+statusTone(weekly)+'"><div><small>HAFTALIK KOÇ</small><h2>'+esc(weekly.headline)+'</h2><p>'+esc(weekly.summary)+'</p></div><div class="sci-score"><b>'+esc(score(weekly))+'</b><span>Ortalama</span><small>Veri güveni '+esc(confidence(weekly))+'</small></div></section>'
  ...
  function renderDesktop(section,date){
    ...
    section.innerHTML='<div id="desktopLegacyCoaching" class="sci-desktop-shell" data-coach-intelligence="1">'+nav(date)
      +'<section class="sci-hero '+statusTone(daily)+'"><div><small>BUGÜNKÜ KARAR</small><h2>'+esc(daily.headline)+'</h2><p>'+esc(daily.summary)+'</p><div class="sci-pills"><span>'+esc(decision(daily))+'</span><span>'+esc(adjustment(daily))+'</span></div></div><div class="sci-score"><b>'+esc(score(daily))+'</b><span>Readiness</span><small>Veri güveni '+esc(confidence(daily))+'</small></div></section>'
      +'<div class="sci-desktop-grid"><section class="sci-card"><header><small>NEDEN?</small><h3>Kararın driver’ları</h3></header>'+list(daily.keyDrivers,'Driver birikiyor.',6)+'</section><section class="sci-card sci-action"><header><small>PRE-WORKOUT</small><h3>'+esc(decision(pre))+'</h3></header><p>'+esc(pre.summary)+'</p><strong>'+esc(adjustment(pre))+'</strong></section><section class="sci-card sci-warning"><header><small>SAFETY</small><h3>AI’dan bağımsız koruma</h3></header>'+list(daily.warnings,'Belirgin risk uyarısı yok.',6)+'</section></div>'
      +'<section class="sci-card"><header><small>HAREKET REHBERİ</small><h3>Main / Accessory / Stability / Conditioning</h3></header>'+guidance(pre)+'</section>'
