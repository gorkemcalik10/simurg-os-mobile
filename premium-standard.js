(function(){
  ...
  function homeModel(date){
    ...
    var stages=polarSleep?{deepMinutes:firstNumber(polarSleep.deepSleepMinutes,secondsToMinutes(polarSleep.deepSleep)),remMinutes:firstNumber(polarSleep.remSleepMinutes,secondsToMinutes(polarSleep.remSleep)),lightMinutes:firstNumber(polarSleep.lightSleepMinutes,secondsToMinutes(polarSleep.lightSleep)),awakeMinutes:firstNumber(polarSleep.awakeTimeMinutes,secondsToMinutes(polarSleep.awakeTime))}:(bridge.sleepStages||recovery.sleepStages||null);
    var sleepStart=text(polarSleep&&firstTextValue(polarSleep.sleepStartTime,polarSleep.sleepStart,polarSleep.bedtimeStart)||bridge.sleepStartTime||recovery.sleepStartTime,'');
    var sleepEnd=text(polarSleep&&firstTextValue(polarSleep.sleepEndTime,polarSleep.sleepEnd,polarSleep.wakeTime)||bridge.sleepEndTime||recovery.sleepEndTime,'');
    ...
    return {selectedDate:date,data:data,bridge:bridge,recovery:recovery,polar:polar,apple:apple,gym:gym,workoutSource:chosen&&chosen.source,polarSleep:polarSleep,polarNightly:polarNightly,loadResult:loadResult,sleepMinutes:sleepMinutes,sleepScore:sleepScore,sleepStart:sleepStart,sleepEnd:sleepEnd,nightly:nightly,nightlyDisplay:nightlyDisplay,readiness:readiness,readinessResult:readinessResult,hasRecoverySignals:hasRecoverySignals,hrv:hrv,rhr:rhr,sleepHr:sleepHr,respiratory:respiratory,load:load,activeEnergy:activeEnergy,rpe:rpe,stages:stages,activity:activity};
  }
  function firstTextValue(){for(var i=0;i<arguments.length;i++){var value=String(arguments[i]==null?'':arguments[i]).trim();if(value)return value;}return '';}
  ...
  function homeReferenceDateLabel(){var value=homeDateValue(),dateLabel;try{dateLabel=new Intl.DateTimeFormat('tr-TR',{day:'numeric',month:'short',year:'numeric',timeZone:'UTC'}).format(new Date(value+'T12:00:00Z'));}catch(e){dateLabel=formatDate(value);}var timeLabel;try{timeLabel=new Intl.DateTimeFormat('tr-TR',{hour:'2-digit',minute:'2-digit'}).format(new Date());}catch(e){timeLabel='';}return [dateLabel,timeLabel].filter(Boolean).join(' · ');}
  ...
  function referenceRing(label,value,color,unit,featured,status){
    var pct=value==null?0:clamp(value,0,100);
    return '<div class="gp-ref-ring-card '+(featured?'featured':'')+(value==null?' unavailable':'')+'"><div class="gp-ref-ring" style="--gp-ref-value:'+pct+'%;--gp-ref-color:'+color+'"><div><b>'+esc(value==null?'—':formatLoad(value))+'</b><small>'+esc(unit||'')+'</small></div></div><span>'+esc(label)+'</span><small class="gp-ref-ring-status">'+esc(status||'Bekleniyor')+'</small></div>';
  }
  function signalHistory(date,key){
    var rows=[];
    if(!window.SimurgSignalModel||typeof window.SimurgSignalModel.day!=='function')return rows;
    for(var i=-6;i<=0;i++){
      var dayDate;try{dayDate=addDays(date,i);}catch(e){continue;}
      var day;try{day=window.SimurgSignalModel.day(dayDate);}catch(e){day=null;}
      if(!day)continue;
      var value=null;
      if(key==='readiness')value=number(day.readiness&&day.readiness.score);
      if(key==='load')value=day.load&&day.load.available?number(day.load.value):null;
      if(value!=null)rows.push({date:dayDate,value:value});
    }
    return rows;
  }
  function trendChart(rows,tone,title){
    if(!rows||rows.length<2)return '<div class="gp-ref-empty">Bu tarih aralığında gerçek trend verisi yok.</div>';
    var values=rows.map(function(row){return row.value;}),min=Math.min.apply(null,values),max=Math.max.apply(null,values);if(max===min){min-=1;max+=1;}
    var points=rows.map(function(row,index){var x=18+(index/(rows.length-1))*264,y=96-((row.value-min)/(max-min))*66;return x.toFixed(1)+','+y.toFixed(1);}).join(' ');
    return '<div class="gp-ref-trend '+tone+'"><svg viewBox="0 0 300 116" role="img" aria-label="'+esc(title)+'"><defs><linearGradient id="gpTrendFill-'+tone+'" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-opacity=".22"></stop><stop offset="100%" stop-opacity="0"></stop></linearGradient></defs><line x1="18" y1="96" x2="282" y2="96"></line><polygon class="gp-ref-trend-area" points="18,96 '+points+' 282,96" fill="url(#gpTrendFill-'+tone+')"></polygon><polyline points="'+points+'"></polyline>'+rows.map(function(row,index){var x=18+(index/(rows.length-1))*264,y=96-((row.value-min)/(max-min))*66;return '<circle cx="'+x.toFixed(1)+'" cy="'+y.toFixed(1)+'" r="3"></circle><text x="'+x.toFixed(1)+'" y="'+(y-8).toFixed(1)+'">'+esc(formatLoad(row.value))+'</text>';}).join('')+'</svg><div>'+rows.map(function(row){return '<span>'+esc(activityDate(row.date,false))+'</span>';}).join('')+'</div></div>';
  }
  function referenceGymCard(model){
    var session=selectedGymSession(model),action="simurgV8Go('workout','logger')";
    if(!session)return '<button type="button" class="gp-ref-session gym empty" onclick="'+action+'"><span class="gp-ref-session-icon">⌘</span><div><small>SEÇİLİ GÜN ANTRENMANI</small><strong>Kayıt bulunmuyor</strong><span>Gym günlüğünü aç</span></div><i>›</i></button>';
    return '<button type="button" class="gp-ref-session gym" onclick="'+action+'"><span class="gp-ref-session-icon">⌘</span><div><small>SEÇİLİ GÜN ANTRENMANI</small><strong>'+esc(session.name)+'</strong><span class="gp-ref-session-date">'+esc(activityDate(session.date,false))+'</span></div><span class="gp-ref-session-metrics"><span><b>'+esc(session.sets)+'</b><em>Set</em></span><span><b>'+esc(session.reps==null?'—':session.reps)+'</b><em>Tekrar</em></span><span class="volume"><b>'+esc(session.volume==null?'—':Math.round(session.volume).toLocaleString('tr-TR')+' kg')+'</b><em>Hacim</em></span></span><i>›</i></button>';
  }
  function referenceActivityCard(model){
    var activity=model.activity;
    if(!activity)return '<div class="gp-ref-session activity empty"><span class="gp-ref-session-icon">⌁</span><div><small>SEÇİLİ GÜN AKTİVİTESİ</small><strong>Aktivite bulunmuyor</strong><span>Polar veya Apple Health kaydı bekleniyor</span></div></div>';
    var activityBody='<span class="gp-ref-session-icon">⌁</span><div><small>SEÇİLİ GÜN AKTİVİTESİ</small><strong>'+esc(activity.name)+'</strong><span class="gp-ref-session-date">'+esc([activity.startTime,activity.duration].filter(Boolean).join(' · '))+'</span></div><span class="gp-ref-session-metrics activity"><span><b>'+esc(activity.cal==null?'—':formatLoad(activity.cal))+'</b><em>kcal</em></span><span><b>'+esc(activity.avgHR==null?'—':formatLoad(activity.avgHR))+'</b><em>Ort. HR</em></span><span><b>'+esc(activity.source||'—')+'</b><em>Kaynak</em></span></span>';
    if(activity.polar)return '<button type="button" class="gp-ref-session activity polar-source" data-gp-polar-date="'+esc(activity.date)+'" data-gp-polar-start="'+esc(activity.startTime||'')+'">'+activityBody+'<i>›</i></button>';
    return '<div class="gp-ref-session activity">'+activityBody+'</div>';
  }
  function exactOverviewRing(label,value,color,unit,status,featured){
    var available=value!=null,pct=available?clamp(value,0,100):0;
    return '<div class="simurgReferenceMetric '+(featured?'featured ':'')+(available?'':'unavailable')+'"><div class="simurgReferenceMetricRing" style="--reference-value:'+pct+'%;--reference-color:'+color+'"><div><b>'+esc(available?formatLoad(value):'—')+'</b>'+(available&&unit?'<small>'+esc(unit)+'</small>':'')+'</div></div><strong>'+esc(label)+'</strong>'+(available&&status?'<span>'+esc(status)+'</span>':'')+'</div>';
  }
  function exactOverviewWorkout(model){
    var session=selectedGymSession(model),action="simurgV8Go('workout','logger')";
    if(!session)return '<button type="button" class="simurgReferenceSession workout empty" onclick="'+action+'"><header><small>SEÇİLİ GÜN ANTRENMANI</small><i>›</i></header><div class="simurgReferenceEmpty"><span>⌘</span><p><b>Kayıt bulunmuyor</b><em>Gym günlüğünü aç</em></p></div></button>';
    return '<button type="button" class="simurgReferenceSession workout" onclick="'+action+'"><header><small>SEÇİLİ GÜN ANTRENMANI</small><i>›</i></header><div class="simurgReferenceSessionTitle"><span>⌘</span><p><b>'+esc(session.name)+'</b><em>'+esc(activityDate(session.date,false))+'</em></p></div><div class="simurgReferenceSessionMetrics"><span><b>'+esc(session.sets)+'</b><small>Set</small></span><span><b>'+esc(session.reps==null?'—':session.reps)+'</b><small>Tekrar</small></span><span class="volume"><b>'+esc(session.volume==null?'—':Math.round(session.volume).toLocaleString('tr-TR')+' kg')+'</b><small>Hacim</small></span></div></button>';
  }
  function exactOverviewActivity(model){
    var activity=model.activity;
    if(!activity)return '<div class="simurgReferenceSession activity empty"><header><small>SEÇİLİ GÜN AKTİVİTESİ</small></header><div class="simurgReferenceEmpty"><span>⌁</span><p><b>Kayıt bulunmuyor</b><em>Polar veya Apple Health bekleniyor</em></p></div></div>';
    var body='<header><small>SEÇİLİ GÜN AKTİVİTESİ</small><i>›</i></header><div class="simurgReferenceSessionTitle"><span>⌁</span><p><b>'+esc(activity.name)+'</b><em>'+esc([activity.startTime,activity.duration].filter(Boolean).join(' · '))+'</em></p></div><div class="simurgReferenceSessionMetrics activity"><span><b>'+esc(activity.cal==null?'—':formatLoad(activity.cal))+'</b><small>kcal</small></span><span><b>'+esc(activity.avgHR==null?'—':formatLoad(activity.avgHR))+'</b><small>Ort. HR</small></span><span><b>'+esc(activity.source||'—')+'</b><small>Kaynak</small></span></div>';
    if(activity.polar)return '<button type="button" class="simurgReferenceSession activity polar-source" data-gp-polar-date="'+esc(activity.date)+'" data-gp-polar-start="'+esc(activity.startTime||'')+'">'+body+'</button>';
    return '<div class="simurgReferenceSession activity">'+body+'</div>';
  }
  function overviewPane(model){
    ...
    var decision=readinessDecision(model),score=model.readiness==null?'—':Math.round(model.readiness),ringColor='#19baf2',overviewLoad=model.loadResult&&model.loadResult.available?model.load:null,
      recoveryStatus=model.readiness==null?'Bekleniyor':model.readiness>=70?'İyi':model.readiness>=55?'Kontrollü':'Düşük',
      sleepStatus=model.sleepScore==null?'Bekleniyor':model.sleepScore>=80?'İyi':model.sleepScore>=60?'Kontrollü':'Düşük',
      loadStatus=overviewLoad==null?'Bekleniyor':(model.loadResult.statusLabel||'Mevcut');
    return '<div class="gp-home-pane active gp-reference-overview simurgReferenceOverview" data-home-pane="overview">'
      +'<section class="simurgReferenceReadiness '+decision.tone+'"><div><small>HAZIRLIK DURUMU</small><strong>'+esc(score)+'<em>/100</em></strong><h2>'+esc(decision.label)+'</h2></div><div class="simurgReferenceReadinessRing" style="--reference-value:'+(model.readiness==null?0:clamp(model.readiness,0,100))+'%;--reference-color:'+ringColor+'"><div></div></div></section>'
      +'<section class="simurgReferenceHorizon"><small>HORIZON METRİKLERİ</small><div>'+exactOverviewRing('Toparlanma',model.readiness,'#32d17b','/100',recoveryStatus,false)+exactOverviewRing('Uyku',model.sleepScore,'#8557e8','/100',sleepStatus,true)+exactOverviewRing('Yük',overviewLoad,'#f7a800','',loadStatus,false)+'</div></section>'
      +'<button type="button" class="simurgReferencePlan" onclick="simurgV8Go(\'gym\',\'gym\')"><span>⌘</span><div><small>BUGÜNKÜ PLAN</small><strong>'+esc(planName(model))+'</strong><p>Programı koru ve çalışma setlerine kontrollü başla.</p></div><i>›</i></button>'
      +'<div class="simurgReferenceSessions">'+exactOverviewWorkout(model)+exactOverviewActivity(model)+'</div>'
      +'<section class="simurgReferenceCoach"><header><small>KOÇUN BUGÜN SENİN İÇİN</small><button type="button" onclick="simurgOpenAlert()" aria-label="Simurg Uyarısı detayları">›</button></header><div class="target"><i>◎</i><p><b>Ana Hedef</b><span>'+esc(planName(model))+' planında kaliteli tekrar ve tam hareket açıklığı.</span></p></div><div class="attention"><i>◇</i><p><b>Dikkat</b><span>'+esc(recoveryInterpretation(model))+'</span></p></div><div class="opportunity"><i>↗</i><p><b>Fırsat</b><span>'+esc(loadInterpretation(model))+'</span></p></div></section>'
      +'</div>';
  }
  function recoveryPane(model){
    ...
    if(window.innerWidth>900)return '<div class="gp-home-pane active" data-home-pane="recovery"><div class="gp-card"><div class="gp-hero-row"><div class="gp-ring gp-hero-ring" style="--gp-value:'+(model.readiness==null?0:clamp(model.readiness,0,100))+'%;--gp-ring-color:'+(model.readiness==null?'#64748b':'#80c72e')+'"><div><b>'+esc(model.readiness==null?'—':Math.round(model.readiness))+'</b><small>'+(model.readiness==null&&model.hasRecoverySignals?'SİNYALLER':'Toparlanma')+'</small></div></div><div class="gp-hero-copy"><small class="gp-kicker">Toparlanma</small><h2>'+esc(title)+'</h2><p>'+esc(recoveryInterpretation(model))+'</p></div></div></div>'+(model.hasRecoverySignals||model.readiness!=null?'<div class="gp-card"><div class="gp-card-head"><h2>Toparlanma Metrikleri</h2><span>'+(model.polarNightly?'Polar AccessLink':'Polar')+'</span></div><div class="gp-metric-grid">'+metric('Gece Toparlanması',model.nightlyDisplay,'')+metric('HRV',model.hrv,'ms')+metric('Dinlenik HR',model.rhr,'bpm')+metric('Solunum Hızı',model.respiratory,'/dk')+metric('Uyku HR',model.sleepHr,'bpm')+metric('Toparlanma Skoru',model.readiness,'/100')+'</div><div class="gp-interpret">'+esc(recoveryInterpretation(model))+'</div></div>':'')+'</div>';
    var history=signalHistory(model.selectedDate,'readiness');
    return '<div class="gp-home-pane active gp-reference-recovery" data-home-pane="recovery"><section class="gp-ref-recovery-hero"><div class="gp-ref-score-ring" style="--gp-ref-value:'+(model.readiness==null?0:clamp(model.readiness,0,100))+'%"><div><b>'+esc(model.readiness==null?'—':Math.round(model.readiness))+'</b><small>/100</small></div></div><div><small>TOPARLANMA SKORU</small><h2>'+esc(title)+'</h2><p>'+esc(recoveryInterpretation(model))+'</p></div></section><section class="gp-ref-metric-section"><header><small>TOPARLANMA METRİKLERİ</small></header><div class="gp-ref-metric-grid">'+metric('HRV',model.hrv,'ms')+metric('Dinlenik HR',model.rhr,'bpm')+metric('Solunum Hızı',model.respiratory,'/dk')+metric('Gece Toparlanması',model.nightlyDisplay,'')+metric('Uyku Kalitesi',model.sleepScore,'/100')+metric('Uyku HR',model.sleepHr,'bpm')+'</div></section><section class="gp-ref-chart-card"><header><small>TOPARLANMA TRENDİ (7 GÜN)</small></header>'+trendChart(history,'recovery','Yedi günlük toparlanma trendi')+'</section></div>';
  }
  ...
  function sleepPane(model){
    if(window.innerWidth>900)return '<div class="gp-home-pane active" data-home-pane="sleep"><div class="gp-card"><div class="gp-hero-row"><div class="gp-ring gp-hero-ring" style="--gp-value:'+(model.sleepScore==null?0:clamp(model.sleepScore,0,100))+'%;--gp-ring-color:#168ed5"><div><b>'+esc(model.sleepScore==null?'—':Math.round(model.sleepScore))+'</b><small>Uyku</small></div></div><div class="gp-hero-copy"><small class="gp-kicker">Uyku Özeti</small><h2>'+esc(formatSleep(model.sleepMinutes))+'</h2><p>'+esc(sleepInterpretation(model))+'</p></div></div></div><div class="gp-card"><div class="gp-card-head"><h2>Uyku Evreleri</h2><span>'+esc(model.sleepMinutes==null?'Bekleniyor':formatSleep(model.sleepMinutes))+'</span></div>'+stageBar(model.stages)+'<div class="gp-interpret">'+esc(sleepInterpretation(model))+'</div></div></div>';
    var hasTimeline=!!(model.sleepStart&&model.sleepEnd);
    return '<div class="gp-home-pane active gp-reference-sleep" data-home-pane="sleep"><section class="gp-ref-sleep-hero"><div><small>UYKU ÖZETİ</small><strong>'+esc(formatSleep(model.sleepMinutes))+'</strong><span class="gp-ref-sleep-badge">'+esc(model.sleepScore==null?'Bekleniyor':Math.round(model.sleepScore)+'/100')+'</span><p>'+esc(sleepInterpretation(model))+'</p></div><div class="gp-ref-night-art" aria-hidden="true"><i></i><b></b></div></section><section class="gp-ref-sleep-stages"><header><small>UYKU EVRELERİ</small><b>'+esc(formatSleep(model.sleepMinutes))+'</b></header>'+stageBar(model.stages)+'</section>'+(hasTimeline?'<section class="gp-ref-timeline"><header><small>UYKU ZAMAN ÇİZELGESİ</small></header><div><b>'+esc(model.sleepStart)+'</b><span></span><b>'+esc(model.sleepEnd)+'</b></div></section>':'')+'<div class="gp-ref-sleep-summary"><div><small>UYKU SKORU</small><b>'+esc(model.sleepScore==null?'—':Math.round(model.sleepScore))+'<em>/100</em></b></div><div><small>VERİ KAYNAĞI</small><b>'+esc(model.polarSleep?'Polar':'Bekleniyor')+'</b></div></div></div>';
  }
  function loadPane(model){
    ...
    if(window.innerWidth>900)return '<div class="gp-home-pane active" data-home-pane="load"><div class="gp-card"><div class="gp-card-head"><h2>Yük Özeti</h2><span class="gp-status '+tone+'">'+statusLabel+'</span></div><div class="gp-metric-grid">'+metrics.join('')+'</div><div class="gp-interpret">'+esc(loadInterpretation(model))+'</div></div>'+(model.activity?activityCard(model.activity,'Son Antrenman / Aktivite',true):'')+'</div>';
    var history=signalHistory(model.selectedDate,'load'),decision=readinessDecision(model);
    if(!loadResult.available)return '<div class="gp-home-pane active gp-reference-load" data-home-pane="load"><section class="gp-ref-load-empty"><span aria-hidden="true">⌁</span><div><small>YÜK ÖZETİ</small><h2>Yük verisi bulunmuyor</h2><p>Seçili gün için doğrulanmış aktivite yükü mevcut değil.</p></div><b>BEKLENİYOR</b></section>'+(history.length>1?'<section class="gp-ref-chart-card"><header><small>YÜK TRENDİ (7 GÜN)</small></header>'+trendChart(history,'load','Yedi günlük yük trendi')+'</section>':'')+'<div class="gp-ref-load-status"><div><small>AGRESİFLİK</small><b>Bekleniyor</b></div><div><small>TREND</small><b>'+esc(loadResult.statusLabel||'Veri yok')+'</b></div><div><small>HAZIRLIK</small><b>'+esc(decision.label)+'</b></div></div>'+referenceActivityCard(model)+'</div>';
    return '<div class="gp-home-pane active gp-reference-load" data-home-pane="load"><section class="gp-ref-load-hero"><header><small>YÜK ÖZETİ</small><span class="gp-status '+tone+'">'+statusLabel+'</span></header><div class="gp-ref-load-main"><div class="gp-ref-load-ring" style="--gp-ref-value:'+(model.load==null?0:clamp(model.load,0,100))+'%"><div><b>'+esc(model.load==null?'—':formatLoad(model.load))+'</b><small>Aktivite Yükü</small></div></div><div><span><small>AKTİF ENERJİ</small><b>'+esc(model.activeEnergy==null?'—':formatLoad(model.activeEnergy))+'<em> kcal</em></b></span><span><small>RPE</small><b>'+esc(model.rpe==null?'—':formatLoad(model.rpe))+'<em>/10</em></b></span><span><small>YÜK KAYNAĞI</small><b>'+esc(loadResult.available?loadResult.source:'Bekleniyor')+'</b></span></div></div></section><section class="gp-ref-chart-card"><header><small>YÜK TRENDİ (7 GÜN)</small></header>'+trendChart(history,'load','Yedi günlük yük trendi')+'</section><div class="gp-ref-load-status"><div><small>AGRESİFLİK</small><b>'+esc(statusLabel)+'</b></div><div><small>TREND</small><b>'+esc(loadResult.statusLabel||'Bekleniyor')+'</b></div><div><small>HAZIRLIK</small><b>'+esc(decision.label)+'</b></div></div>'+referenceActivityCard(model)+'<div class="gp-ref-load-note">'+esc(loadInterpretation(model))+'</div></div>';
  }
  function homeShell(){
    ...
    if(homeTab==='overview')return '<div class="gp-home-shell simurgReferenceOverviewShell" data-home-layout="mobile-overview"><header class="simurgReferenceHeader"><img src="./icons/icon-192.png" alt="Simurg"><div><small>SIMURG OS</small><h1>Günaydın, Görkem</h1><p><time id="gpHomeDateLabel">'+esc(homeReferenceDateLabel())+'</time><button type="button" aria-label="Önceki gün" onclick="homePremiumMove(-1)">‹</button><button type="button" aria-label="Sonraki gün" onclick="homePremiumMove(1)">›</button></p></div><button type="button" class="simurgReferenceAlert" aria-label="Bildirimler" onclick="simurgOpenAlert()">🔔<i></i></button></header><div class="gp-home-tabs" role="tablist">'+homeTabs.map(function(tab){return '<button class="gp-home-tab '+(tab===homeTab?'active':'')+'" data-home-tab="'+tab+'" role="tab" aria-selected="'+(tab===homeTab?'true':'false')+'" type="button" onclick="homePremiumSetTab(\''+tab+'\')">'+tab.toUpperCase()+'</button>';}).join('')+'</div><div id="gpHomeContent" class="gp-home-content"></div></div>';
  ...
  function renderHome(){
    ...
    var desiredLayout=window.innerWidth>900?'desktop':homeTab==='overview'?'mobile-overview':'mobile',shell=home.querySelector('.gp-home-shell');if(!shell||shell.dataset.homeLayout!==desiredLayout)home.innerHTML=homeShell();
    var dateLabel=document.getElementById('gpHomeDateLabel');if(dateLabel)dateLabel.textContent=desiredLayout==='mobile-overview'?homeReferenceDateLabel():homeDateLabel();
