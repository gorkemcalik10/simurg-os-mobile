(function(){
  ...
  function referenceRing(label,value,color,unit,featured){
    var pct=value==null?0:clamp(value,0,100);
    return '<div class="gp-ref-ring-card '+(featured?'featured':'')+'"><div class="gp-ref-ring" style="--gp-ref-value:'+pct+'%;--gp-ref-color:'+color+'"><div><b>'+esc(value==null?'—':formatLoad(value))+'</b><small>'+esc(unit||'')+'</small></div></div><span>'+esc(label)+'</span></div>';
  }
  ...
  function referenceGymCard(model){
    ...
    return '<button type="button" class="gp-ref-session gym" onclick="'+action+'"><span class="gp-ref-session-icon">⌘</span><div><small>SEÇİLİ GÜN ANTRENMANI</small><strong>'+esc(session.name)+'</strong><span class="gp-ref-session-date">'+esc(activityDate(session.date,false))+'</span><span class="gp-ref-session-metrics"><span><b>'+esc(session.sets)+'</b><em>Set</em></span><span><b>'+esc(session.reps==null?'—':session.reps)+'</b><em>Tekrar</em></span><span><b>'+esc(session.volume==null?'—':Math.round(session.volume).toLocaleString('tr-TR'))+'</b><em>kg</em></span></span></div><i>›</i></button>';
  }
  function referenceActivityCard(model){
    ...
    var activityBody='<span class="gp-ref-session-icon">⌁</span><div><small>SEÇİLİ GÜN AKTİVİTESİ</small><strong>'+esc(activity.name)+'</strong><span class="gp-ref-session-date">'+esc([activity.startTime,activity.duration].filter(Boolean).join(' · '))+'</span><span class="gp-ref-session-metrics activity"><span><b>'+esc(activity.cal==null?'—':formatLoad(activity.cal))+'</b><em>kcal</em></span><span><b>'+esc(activity.avgHR==null?'—':formatLoad(activity.avgHR))+'</b><em>Ort. HR</em></span><span><b>'+esc(activity.source||'—')+'</b><em>Kaynak</em></span></span></div>';
  ...
  function overviewPane(model){
    ...
    var decision=readinessDecision(model),score=model.readiness==null?'—':Math.round(model.readiness),ringColor='#20bbed',overviewLoad=model.loadResult&&model.loadResult.available?model.load:null;
    return '<div class="gp-home-pane active gp-reference-overview" data-home-pane="overview">'
      +'<section class="gp-ref-readiness '+decision.tone+'"><div class="gp-ref-readiness-copy"><small>HAZIRLIK DURUMU</small><strong>'+esc(score)+'<em>/100</em></strong><h2>'+esc(decision.label)+'</h2><span>'+esc((model.readinessResult&&model.readinessResult.confidence&&model.readinessResult.confidence.label)||'Bekleniyor')+'</span></div><div class="gp-ref-readiness-ring" style="--gp-ref-value:'+(model.readiness==null?0:clamp(model.readiness,0,100))+'%;--gp-ref-color:'+ringColor+'"><div><i></i></div></div></section>'
      +'<section class="gp-ref-horizon"><small>HORIZON METRİKLERİ</small><div>'+referenceRing('Toparlanma',model.readiness,'#31cc70','/100',false)+referenceRing('Uyku',model.sleepScore,'#8159dd','/100',true)+referenceRing('Yük',overviewLoad,'#ee9d08','',false)+'</div></section>'
      +'<button type="button" class="gp-ref-plan" onclick="simurgV8Go(\'gym\',\'gym\')"><span>⌘</span><div><small>BUGÜNKÜ PLAN</small><strong>'+esc(planName(model))+'</strong><p>Programı koru ve çalışma setlerine kontrollü başla.</p></div><i>›</i></button>'
      +'<div class="gp-ref-session-grid">'+referenceGymCard(model)+referenceActivityCard(model)+'</div>'
      +'<section class="gp-ref-coach"><header><small>KOÇUN BUGÜN SENİN İÇİN</small><button type="button" onclick="simurgOpenAlert()" aria-label="Simurg Uyarısı detayları">›</button></header><div><i>◎</i><b>Ana Hedef</b><span>'+esc(planName(model))+' planında kaliteli tekrar ve tam hareket açıklığı.</span></div><div><i>◇</i><b>Dikkat</b><span>'+esc(recoveryInterpretation(model))+'</span></div><div><i>↗</i><b>Fırsat</b><span>'+esc(loadInterpretation(model))+'</span></div></section>'
      +'</div>';
  }
