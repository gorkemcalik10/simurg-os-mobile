(function(root){
  ...
  function anatomy(selected,exercise){var highlights=exerciseSelection(exercise),manualSelection=!exercise&&selected&&state.muscle===selected.id&&VISUAL_REGION_MAP[selected.id];if(manualSelection)highlights[selected.id]='primary';var activeIds=Object.keys(highlights),ordered=VISUAL_REGIONS.filter(function(item){return activeIds.indexOf(item.id)<0}).concat(VISUAL_REGIONS.filter(function(item){return highlights[item.id]==='secondary'}),VISUAL_REGIONS.filter(function(item){return highlights[item.id]==='primary'})),label=exercise?exercise.name:manualSelection?selected.label:'Nötr görünüm',description=exercise?exercise.name+' hareketinin ana kasları güçlü kırmızı, yardımcı kasları yumuşak turuncu kırmızıyla vurgulanır.':manualSelection?selected.label+' bölgesi kırmızıyla vurgulanır.':'Bir kas veya hareket seçilene kadar anatomi figürü nötr gösterilir.';return '<figure class="tlAnatomy" data-selected-muscle="'+esc(selected.id)+'"'+(exercise?' data-selected-exercise="'+esc(exercise.exerciseId)+'"':'')+'><div class="tlAnatomyStage"><img src="./assets/simurg-anatomy-base-v1.png" alt="" aria-hidden="true" decoding="async"><svg viewBox="0 0 1024 1536" preserveAspectRatio="xMidYMid meet" aria-labelledby="tlAnatomyTitle tlAnatomyDesc"><title id="tlAnatomyTitle">Ön ve arka fitness kas haritası</title><desc id="tlAnatomyDesc">'+esc(description)+'</desc>'+ordered.map(function(item){return region(item,highlights)}).join('')+'</svg></div><figcaption><span>ÖN</span><b>'+esc(label)+'</b><span>ARKA</span></figcaption></figure>'}
