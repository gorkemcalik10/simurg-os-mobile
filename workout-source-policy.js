(function(){
  ...
  function durationSeconds(value,unitHint){
    if(value==null||value==='')return 0;
    var hint=String(unitHint||'auto').toLowerCase(),n;
    if(typeof value==='number'||/^\d+(?:[.,]\d+)?$/.test(String(value).trim())){
      n=Number(String(value).replace(',','.'));if(!Number.isFinite(n)||n<0)return 0;
      if(hint==='minutes')return n*60;
      if(hint==='milliseconds'||(hint==='auto'&&n>=86400))return n/1000;
      return n;
    }
    var raw=String(value).trim(),iso=raw.match(/^P(?:(\d+(?:\.\d+)?)D)?(?:T(?:(\d+(?:\.\d+)?)H)?(?:(\d+(?:\.\d+)?)M)?(?:(\d+(?:\.\d+)?)S)?)?$/i);
    if(iso)return (Number(iso[1]||0)*86400)+(Number(iso[2]||0)*3600)+(Number(iso[3]||0)*60)+Number(iso[4]||0);
    ...
    return (hours?Number(hours[1].replace(',','.'))*3600:0)+(minutes?Number(minutes[1].replace(',','.'))*60:0)+(seconds?Number(seconds[1].replace(',','.')):0);
  }
  function durationMinutes(value,unitHint){return durationSeconds(value,unitHint)/60;}
  function formatDuration(value,unitHint){
    if(value==null||value==='')return '—';var seconds=Math.max(0,Math.round(durationSeconds(value,unitHint)));
    if(!seconds&&String(value).trim()!=='0')return '—';var h=Math.floor(seconds/3600),m=Math.floor(seconds%3600/60),s=seconds%60;
    return h?String(h)+':'+String(m).padStart(2,'0')+':'+String(s).padStart(2,'0'):String(m).padStart(2,'0')+':'+String(s).padStart(2,'0');
  }
  function rowDurationMinutes(row){if(!row)return 0;return row.duration!=null?durationMinutes(row.duration):durationMinutes(row.durationMinutes,'minutes');}
  ...
  function validApple(row){
    ...
    return [row.activeCal,row.totalCal,row.avgHR,row.maxHR,row.distance,row.steps].some(function(v){return number(v)>0;})||durationMinutes(row.duration!=null?row.duration:row.durationText)>0;
  }
  ...
  function gymDuration(rows){for(var i=0;i<rows.length;i++){var n=rowDurationMinutes(rows[i]);if(n>0)return n;}return 0;}
  function matchScore(workout,gym){
    var score=0,gTime=gymTime(gym),pTime=timeMinutes(workout.startTime||workout.start_time),gDuration=gymDuration(gym),pDuration=rowDurationMinutes(workout),type=fitnessType(workout);
  ...
  function metrics(date){
    var session=day(date),p=session.primaryPolar,a=session.appleLegacy,g=session.gym,cardio=session.cardioLoad||{},physical=p||a||{},minutes=rowDurationMinutes(physical),active=first(physical.activeCal,physical.activeCalories,physical.calories,physical.kilocalories)||0,avgHR=first(physical.avgHR,physical.avgHr,physical.averageHeartRate)||0,maxHR=first(physical.maxHR,physical.maxHr,physical.maximumHeartRate)||0;
    var rpes=g.map(function(x){return number(x.rpe);}).filter(function(x){return x!=null;}),avgRpe=rpes.length?rpes.reduce(function(s,x){return s+x;},0)/rpes.length:0,sets=g.reduce(function(s,x){return s+(number(x.sets)||1);},0),volume=g.reduce(function(s,x){return s+(number(x.weight)||0)*(number(x.reps)||0)*(number(x.sets)||1);},0),painWarning=g.some(function(x){return /warning|severe|high|şiddet/i.test(String(x.pain||''));}),painMild=g.some(function(x){return /mild|hafif|yes|var/i.test(String(x.pain||''));}),badForm=g.some(function(x){return /bad|poor|kötü/i.test(String(x.form||''));});
  ...
  window.SimurgWorkoutSource={day:day,metrics:metrics,latest:latest,allDates:allDates,countSessions:countSessions,validApple:validApple,durationMinutes:durationMinutes,formatDuration:formatDuration};
