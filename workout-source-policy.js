(function(){
  ...
  function metrics(date){
    ...
    var direct=resolvedLoad&&resolvedLoad.available?resolvedLoad.value:first(session.cardioLoad&&session.cardioLoad.cardioLoad,session.cardioLoad&&session.cardioLoad.load,session.cardioLoad&&session.cardioLoad.strain,p&&p.cardioLoad),zoneMinutes=hasPolarAggregate?session.polar.reduce(function(sum,row){return sum+zonesSeconds(row.zones||row.heartRateZones)/60;},0):(p?zonesSeconds(p.zones||p.heartRateZones)/60:0);
