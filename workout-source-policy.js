(function(){
  ...
  function metrics(date){
    var session=day(date),p=session.primaryPolar,a=session.appleLegacy,g=session.gym,physical=p||a||{},resolvedLoad=window.SimurgSignalModel&&typeof window.SimurgSignalModel.load==='function'?window.SimurgSignalModel.load(date):null,official=resolvedLoad&&resolvedLoad.available&&resolvedLoad.source==='Polar Cardio Load'?{available:true,cardioLoad:resolvedLoad.value}:null,aggregate=window.SimurgSignalModel&&typeof window.SimurgSignalModel.polarDayAggregate==='function'?window.SimurgSignalModel.polarDayAggregate(date,session.polar,official):null,hasPolarAggregate=!!(aggregate&&aggregate.sessionCount),minutes=hasPolarAggregate?aggregate.durationMinutes:durationMinutes(physical.duration||physical.durationMinutes),active=hasPolarAggregate?(aggregate.activeCalories==null?0:aggregate.activeCalories):(first(physical.activeCal,physical.activeCalories,physical.calories,physical.kilocalories)||0),avgHR=hasPolarAggregate?(aggregate.avgHR==null?0:aggregate.avgHR):(first(physical.avgHR,physical.avgHr,physical.averageHeartRate)||0),maxHR=hasPolarAggregate?(aggregate.maxHR==null?0:aggregate.maxHR):(first(physical.maxHR,physical.maxHr,physical.maximumHeartRate)||0);
    ...
    var direct=resolvedLoad&&resolvedLoad.available?resolvedLoad.value:first(session.cardioLoad&&session.cardioLoad.cardioLoad,session.cardioLoad&&session.cardioLoad.load,session.cardioLoad&&session.cardioLoad.strain,p&&p.cardioLoad,p&&p.trainingLoad),zoneMinutes=hasPolarAggregate?session.polar.reduce(function(sum,row){return sum+zonesSeconds(row.zones||row.heartRateZones)/60;},0):(p?zonesSeconds(p.zones||p.heartRateZones)/60:0);
    ...
    var types=hasPolarAggregate?Array.from(new Set(session.polar.map(function(row){return String(row.workoutType||row.activityType||row.type||'Workout').replace(/[_-]+/g,' ');}))):(physical?[String(physical.workoutType||physical.activityType||physical.type||'Workout').replace(/[_-]+/g,' ')]:[]);
    return {session:session,watch:session.appleLegacy?[session.appleLegacy]:[],polar:session.polar,polarAggregate:aggregate,gym:{sets:sets,volume:volume},coach:{avgRpe:avgRpe,painWarning:painWarning,painMild:painMild,badForm:badForm},active:active,total:hasPolarAggregate?(aggregate.totalCalories==null?0:aggregate.totalCalories):(first(physical.totalCal,physical.totalCalories)||0),minutes:minutes,avgHR:avgHR,maxHR:maxHR,types:types,loadScore:loadScore,level:level,source:session.source,primary:session.primary,extraPolar:session.extraPolar};
  }
